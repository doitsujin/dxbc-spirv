...
std::ostream& writeToStream(std::ostream& os, RegisterType registerType, ShaderType shaderType, uint32_t shaderVersionMajor) {
  switch (registerType) {
    ...
    // case RegisterType::eTexture: Same value
      return os << (shaderType == ShaderType::eVertex ? "a" : "t");
    ...
    // case RegisterType::eOutput: Same value.
      return os << (shaderVersionMajor == 3 ? "o" : "oT");
  ...
  return os << "Register(" << uint32_t(registerType) << ")";
}
