...
bool RegisterFile::handleDclIndexableTemp(ir::Builder& builder, const Instruction& op) {
   ...
   * (imm0) Array length
   * (imm1) Vector component count. In fxc-generated binaries, this
   *        is always 4, so we will ignore it and optimize later.
   */
  auto dstReg = op.getDst(0u);

  if (dstReg.getRegisterType() != RegisterType::eIndexableTemp || !dstReg.getIndexDimensions())
    return m_converter.logOpError(op, "Invalid operand.");

  auto index = dstReg.getIndex(0u);
  auto arraySize = op.getImm(0u).getImmediate<uint32_t>(0u);
  ...
  if (m_xRegs[index]) {
    auto name = m_converter.makeRegisterDebugName(dstReg.getRegisterType(), index, WriteMask());
