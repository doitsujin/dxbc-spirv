  ...
  switch (op->getOpCode()) {
    ...
    case OpCode::eIXor: {
      ...
      if (isConstantSelect(b)) {
         ...
         * select(c0, a, 0) & select(c1, b, 0) -> select(c0 && c1, a & b, 0)
         * select(c0, a, 0) | select(c1, b, 0) -> select(c0 || c1, a | b, 0)
         * select(c0, a, 0) ^ select(c1, b, 0) -> select(c0 != c1, a | b, 0) */
        ...
        if ((isZeroConstant(m_builder.getOp(at)) || isZeroConstant(m_builder.getOp(af))) &&
            (isZeroConstant(m_builder.getOp(bt)) || isZeroConstant(m_builder.getOp(bf)))) {
          auto ac = SsaDef(a.getOperand(0u));
          auto bc = SsaDef(b.getOperand(0u));

          auto ref = op->getDef();

          /* Arrange A condition so that the zero operand is last */
          if (isZeroConstant(m_builder.getOp(at))) {
            ac = ref = m_builder.addBefore(ref, Op::BNot(ScalarType::eBool, ac));
            std::swap(at, af);
          }

          /* Arrange B condition so that the zero operand is last */
          if (isZeroConstant(m_builder.getOp(bt))) {
            bc = ref = m_builder.addBefore(ref, Op::BNot(ScalarType::eBool, bc));
            std::swap(bt, bf);
          }

          dxbc_spv_assert(af == bf);

          /* Make binary op for non-zero select value */
          auto value = ref = m_builder.addBefore(ref,
            Op(op->getOpCode(), op->getType()).addOperands(at, bt));
          ...
          m_builder.rewriteOp(op->getDef(), Op::Select(op->getType(), cond, value, af));
...
std::pair<bool, Builder::iterator> ArithmeticPass::resolveIdentityArithmeticOp(Builder::iterator op) {
  switch (op->getOpCode()) {
    ...
    case OpCode::eFDiv: {
      ...
      /* 1.0 / a -> rcp(a) */
      if (!(getFpFlags(*op) & OpFlag::ePrecise)) {
        bool isRcp = a.isConstant();

        if (isRcp) {
          for (uint32_t i = 0u; i < a.getOperandCount(); i++)
            isRcp = isRcp && getConstantAsFloat(a, i) == 1.0;
        }

        if (isRcp) {
          m_builder.rewriteOp(op->getDef(), Op::FRcp(op->getType(), b.getDef()));
          return std::make_pair(true, op);
        }
      }
      ...
      if ((a.getOpCode() == negOp || b.getOpCode() == negOp) && !isUInt) {
        ...
        if (aOp.getDef() == bOp.getDef()) {
          ...
          if (op->getOpCode() == OpCode::eFMax || op->getOpCode() == OpCode::eSMax) {
            ...
            m_builder.rewriteOp(op->getDef(),
              Op(negOp, op->getType()).setFlags(op->getFlags()).addOperand(aOp.getDef()));
    ...
    case OpCode::eFRound: {
      ...
      auto roundMode = RoundMode(a.getOperand(a.getFirstLiteralOperandIndex()));
