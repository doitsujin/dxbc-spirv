...
FloatControlPass::FloatControlPass(Builder& builder, const Options& options)
...
void FloatControlPass::run() {
  ...
  propagateSignedZeroPreserve();
  assignOpFlags();
}


void FloatControlPass::runPass(Builder& builder, const Options& options) {
  FloatControlPass(builder, options).run();
}


bool FloatControlPass::gatherFloatValues() {
  bool progress = false;
...
bool FloatControlPass::gatherFloatValuesForOp(const Op& op) {
  ...
  if ((flags.sz ^ oldFlags.sz) & FloatSzMode::ePreserve)
...
FloatControlPass::Flags FloatControlPass::determineFloatValuesForOp(const Op& op) {
  switch (op.getOpCode()) {
...
FloatControlPass::Flags FloatControlPass::handleMinMax(size_t count, const FloatValue* flags, FloatValueFlags a, FloatValueFlags b) {
...
FloatControlPass::Flags FloatControlPass::handleMin(FloatValueFlags a, FloatValueFlags b) {
...
FloatControlPass::Flags FloatControlPass::handleMax(FloatValueFlags a, FloatValueFlags b) {
...
FloatControlPass::Flags FloatControlPass::handleAdd(FloatValueFlags a, FloatValueFlags b) {
  /* Propagate nan */
  Flags result = { };
...
FloatControlPass::Flags FloatControlPass::handleMul(FloatValueFlags a, FloatValueFlags b, bool same) {
  /* Propagate NaN */
  Flags result = { };
  result.value = (a | b) & FloatValue::eNan;
...
FloatControlPass::Flags FloatControlPass::handleRcp(FloatValueFlags a) {
...
FloatControlPass::Flags FloatControlPass::handleSqrt(FloatValueFlags a) {
  Flags result = { };
...
void FloatControlPass::preserveSignedZero(const Op& op) {
  ...
  m_signedZeroPreserveOps.push_back(op.getDef());
}
...
OpFlags FloatControlPass::determineOpFlags(const Op& op) {
  auto flags = m_flags.at(op.getDef());
  ...
  if (!(flags.value & FloatValue::eNan))
    result |= OpFlag::eNoNan;

  if (!(flags.value & (FloatValue::eNegInf | FloatValue::ePosInf)))
    result |= OpFlag::eNoInf;

  if (!(flags.value & FloatValue::eNeg0))
    result |= OpFlag::eNoSz;
...
void FloatControlPass::propagateSignedZeroPreserve() {
  while (!m_signedZeroPreserveOps.empty()) {
    const auto& op = m_builder.getOp(m_signedZeroPreserveOps.back());
    m_signedZeroPreserveOps.pop_back();

    /* If the instruction eliminates signed zero inputs, don't
     * tag operands for preserving signed zero. */
    const auto& opFlags = m_flags.at(op.getDef());

    if (opFlags.sz & FloatSzMode::eEliminate)
      continue;

    /* Recursively tag operands that actually return a value */
...
bool FloatControlPass::isFloatOp(const Op& op) {
